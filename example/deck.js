...
var Deck = (function () {
  ...
  var flip = {
    deck: function deck(_deck2) {
      _deck2.flip = _deck2.queued(flip);

      function flip(next, side) {
        var flipped = _deck2.cards.filter(function (card) {
          return card.side === 'front';
        }).length / _deck2.cards.length;
  ...
  var sort = {
    deck: function deck(_deck3) {
      _deck3.sort = _deck3.queued(sort);
    ...
    card: function card(_card3) {
      ...
      _card3.sort = function (i, len, cb, reverse) {
        var z = i / 4;
        var delay = i * 10;
  ...
  Deck.ease = ease;
  Deck.modules = { bysuit: bysuit, fan: fan, intro: intro, poker: poker, shuffle: shuffle, sort: sort, flip: flip, cao: cao };
  Deck.Card = _card;
  Deck.prefix = prefix;
